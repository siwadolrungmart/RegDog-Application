...
class AddVetVisitEventPage extends StatefulWidget {
  final DateTime selectedDateFromCalendar;
  final String? eventId; 
  final Map<String, dynamic>? eventData; 

  const AddVetVisitEventPage({
    super.key,
  ...
  @override
  State<AddVetVisitEventPage> createState() => _AddVetVisitEventPageState();
}

class _AddVetVisitEventPageState extends State<AddVetVisitEventPage> {
  final FirebaseFirestore _firestore = FirebaseFirestore.instance;
  
  // 🟢 เปลี่ยนชื่อ Default เป็น "พบสัตวแพทย์"
  final TextEditingController _nameController = TextEditingController(text: "พบสัตวแพทย์");
  ...
  void initState() {
    ...
    if (widget.eventData != null) {
      ...
      _nameController.text = data['name'] ?? "พบสัตวแพทย์";
                      ...
                      Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            _buildFormRow(
                              "ชื่อ:", 
                              _buildInputBox(_nameController, "ชื่อกิจกรรม/อาการ"), 
                              textLabelBlue
                            ),
                            _buildDivider(),
  ...
  Future<void> _deleteEvent() async {
    bool confirm = await showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text("คุณแน่ใจหรือไม่ว่าต้องการลบข้อมูลการพบสัตวแพทย์นี้?", style: GoogleFonts.inter()),
        actions: [
  ...
  Future<void> _saveToFirebase() async {
    ...
    try {
      ...
      for (File imageFile in _selectedLocalImages) {
        String fileName = 'health/${DateTime.now().millisecondsSinceEpoch}_${imageFile.path.split('/').last}';
      ...
      await NotificationService.scheduleEventNotification(
        eventId: targetId,
        title: "แจ้งเตือนพบสัตวแพทย์: ${_nameController.text}",
        body: "ถึงเวลานัดหมายพบสัตวแพทย์แล้ว",
    ...
    return Row(
      children: [
        ...
        Expanded(
          child: Center(
            child: Text(
              widget.eventId == null ? "เพิ่มข้อมูลสุขภาพ" : "รายละเอียดการพบสัตวแพทย์", 
  ...
  Widget _buildActivityImages(Color bg, Color iconCol) {
        ...
        Padding(
          ...
          child: GestureDetector(
            ...
            child: Stack(
              ...
              children: [
                // 🟢 เปลี่ยนมาใช้ Icons.domain (รูปตึก) ตรงนี้ครับ
                Container(width: 75, height: 75, decoration: BoxDecoration(color: bg, shape: BoxShape.circle), child: Icon(Icons.domain, color: iconCol, size: 40)),
    ...
    return Column(
      children: [
        ...
        Text("พบสัตวแพทย์", style: GoogleFonts.inter(color: iconCol, fontWeight: FontWeight.bold, fontSize: 16))
      ]
    );
  }
  ...
  // 🟢 อัปเดต _buildInputBox ให้รับค่า action และ maxLines ได้
