...
class AddSymptomEventPage extends StatefulWidget {
  final DateTime selectedDateFromCalendar;
  final String? eventId; 
  final Map<String, dynamic>? eventData; 

  const AddSymptomEventPage({
    super.key,
  ...
  @override
  State<AddSymptomEventPage> createState() => _AddSymptomEventPageState();
}
...
class _AddSymptomEventPageState extends State<AddSymptomEventPage> {
  ...
  Future<void> _deleteEvent() async {
    bool confirm = await showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text("คุณแน่ใจหรือไม่ว่าต้องการลบข้อมูลอาการนี้?", style: GoogleFonts.inter()),
        actions: [
  ...
  Future<void> _saveToFirebase() async {
    if (_nameController.text.trim().isEmpty) {
      _showErrorSnackBar("กรุณาระบุชื่ออาการ");
      return;
    }
    ...
    try {
      ...
      for (File imageFile in _selectedLocalImages) {
        String fileName = 'symptoms/${DateTime.now().millisecondsSinceEpoch}_${imageFile.path.split('/').last}';
      ...
      if (mounted) {
        ...
        Navigator.pop(context); 
        Navigator.pop(context); 
        messenger.showSnackBar(const SnackBar(content: Text("บันทึกข้อมูลเรียบร้อยแล้ว"), backgroundColor: Colors.green));
      }
    } catch (e) {
  ...
  Widget _buildHeader(BuildContext context) {
    return Row(
      children: [
        ...
        Expanded(child: Center(child: Text(widget.eventId == null ? "เพิ่มข้อมูลสุขภาพ" : "รายละเอียดอาการ", style: GoogleFonts.inter(fontSize: 16, fontWeight: FontWeight.w500)))),
  ...
  Widget _buildActivityImages(Color bg, Color iconCol) {
        ...
        Padding(
          ...
          child: GestureDetector(
            ...
            child: Stack(
              ...
              children: [
                Container(width: 75, height: 75, decoration: BoxDecoration(color: bg, shape: BoxShape.circle), child: Icon(Icons.note_alt_outlined, color: iconCol, size: 40)),
    ...
    return Column(children: [SingleChildScrollView(scrollDirection: Axis.horizontal, child: Row(mainAxisAlignment: MainAxisAlignment.center, children: imageWidgets)), const SizedBox(height: 10), Text("อาการ", style: GoogleFonts.inter(color: iconCol, fontWeight: FontWeight.bold, fontSize: 16))]);
  }
