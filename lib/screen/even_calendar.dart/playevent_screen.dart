...
// 🔴 ลบ Import distance_picker.dart ออก เพราะไม่ได้ใช้
...
class AddPlayEventPage extends StatefulWidget {
  final DateTime selectedDateFromCalendar;
  final String? eventId; 
  final Map<String, dynamic>? eventData; 

  const AddPlayEventPage({
    super.key,
  ...
  @override
  State<AddPlayEventPage> createState() => _AddPlayEventPageState();
}

class _AddPlayEventPageState extends State<AddPlayEventPage> {
  final FirebaseFirestore _firestore = FirebaseFirestore.instance;
  
  // 🟢 เปลี่ยนชื่อ Default เป็น "เวลาเล่น"
  final TextEditingController _nameController = TextEditingController(text: "เวลาเล่น");
  final TextEditingController _noteController = TextEditingController();
  
  TimeOfDay _selectedTime = TimeOfDay.now(); 
  Duration? _selectedDuration;
  // 🔴 ลบตัวแปร _selectedDistance ออก
  ...
  void initState() {
    ...
    if (widget.eventData != null) {
      ...
      _nameController.text = data['name'] ?? "เวลาเล่น";
      ...
      // 🔴 ลบการดึงข้อมูล distance_km ออก
  ...
  Future<void> _saveToFirebase() async {
    ...
    try {
      ...
      Map<String, dynamic> payload = {
        'type': 'play', // 🟢 เปลี่ยน Type เป็น play
        'dog_id': currentDogId, 
  ...
  Widget _buildActivityImages(Color bg, Color iconCol) {
        ...
        Padding(
          ...
          child: GestureDetector(
            ...
            child: Stack(
              ...
              children: [
                // 🟢 เปลี่ยน Icon เป็น sports_volleyball ให้ตรงกับเวลาเล่น
                Container(width: 75, height: 75, decoration: BoxDecoration(color: bg, shape: BoxShape.circle), child: Icon(Icons.sports_volleyball, color: iconCol, size: 40)),
    ...
    // 🟢 เปลี่ยน Text เป็น "เวลาเล่น"
    return Column(children: [SingleChildScrollView(scrollDirection: Axis.horizontal, child: Row(mainAxisAlignment: MainAxisAlignment.center, children: imageWidgets)), const SizedBox(height: 10), Text("เวลาเล่น", style: GoogleFonts.inter(color: iconCol, fontWeight: FontWeight.bold, fontSize: 16))]);
  }
  ...
  // 🔴 ลบ _buildDistancePickerBtn ออก เพราะไม่ได้ใช้แล้ว
